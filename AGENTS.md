...
- **R-1.6** **사용자는 비개발자다.** 사용자에게 하는 설명은 전문용어 없이 쓴다.
  - 전문용어를 꼭 써야 하면 **처음 나올 때 한 번** 풀어 쓴다. 예: "CI(코드를 고칠 때마다 깃허브 서버가 자동으로 컴파일해보는 장치)".
  - 도구 이름·플래그·파일 경로를 나열하는 것으로 설명을 대신하지 않는다. **무엇이 어떻게 좋아지는지**를 먼저 말한다.
  - 사용자가 직접 해야 하는 일이 있으면 **어디서 무엇을 누르는지** 단계로 적는다. 없으면 "사용자님이 하실 일은 없습니다"라고 명시한다.
  - 사용자의 컴퓨터·게임 설치본이 필요한 작업인지 아닌지를 먼저 밝힌다.
  - 코드 주석·커밋 메시지·로그는 이 규칙의 대상이 아니다 (R-1.1).
